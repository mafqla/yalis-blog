...
编译代码的时候，如果遇到以 `'use client'` 为开头的组件，说明是客户端组件，使用特殊的占位符替代。比如以目前的 JSX 对象实现方式为例的话，可以替换为 `<code>&lt;div data-client="client" data-component="Like"&gt;</code>` 这样的节点。表明该位置是客户端组件占位，指向的客户端组件是 `<code>&lt;Like&gt;</code>`。
...
在 `transformClientComponent` 中，最后我们会返回一个 `React.createElement( "div", { "data-client": true, "data-component": Component.name})` 的 JSX 对象，它会被渲染为 `<code>&lt;div data-client="client" data-component="Like"&gt;</code>`，用于客户端组件占位。
...
> 1.  功能实现：客户端组件
> 2.  源码地址：[https://github.com/mqyqingfeng/next-app-demo/tree/react-rsc-12](https://github.com/mqyqingfeng/next-app-demo/tree/react-rsc-12)
...
在 Next.js 中的实现，客户端组件其实是会被预渲染的，而我们是直接替换为一个 `<code>&lt;div data-client="client" data-component="Like"&gt;</code>` DOM 节点。因为 DOM 节点无内容，所以我们直接用了 createRoot，更贴合的实现应该用 hyrateRoot。
