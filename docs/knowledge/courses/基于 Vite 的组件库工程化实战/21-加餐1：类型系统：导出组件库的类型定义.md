...
![img](images\ef10aa25a839431b8ff89722c677de53~tplv-k3u1fbpfcp-zoom-1.jpg)
...
![img](images\cca14f674153425fae0cc4517a39a9dc~tplv-k3u1fbpfcp-zoom-1.jpg)

但是如果将 utils.ts 编译为 utils.js 文件。这个时候类型定义就会丢失。

![img](images\76dc13cfede84a8ba12116efc9984e6f~tplv-k3u1fbpfcp-zoom-1.jpg)

那调用的时候自然也不会有类型检查。

![img](images\cfd961b295fa4623bc181c05c3733e15~tplv-k3u1fbpfcp-zoom-1.jpg)

可以看到调用 add 方法的时候，类型是 any。

如果想要保留类型定义，就需要生成类型定义文件，将类型保留到类型定义文件中 d.ts。这个时候编辑器就可以依据类型定义文件进行类型检查了。

![img](images\c1532941d53249a99bbfc8f849cbcb49~tplv-k3u1fbpfcp-zoom-1.jpg)
...
![img](images\66c19c7ffb7a4c80a22eed9c519f2aa1~tplv-k3u1fbpfcp-zoom-1.jpg)
...
![img](images\f1f2e6251b3941b193dd818af9e2c0d3~tplv-k3u1fbpfcp-zoom-1.jpg)
...
-   copyDtsFiles： 目的是可以自动复制源码中的类型定义，这个需要有。
    ...
    -   ```
        pnpm build
        ```
    -   ![img](images\6213341d00184ae689ec1ebdfc156250~tplv-k3u1fbpfcp-zoom-1.jpg)

### 定义类型定义入口

类型定义入口文件是靠编写编译脚本实现的。如何编写这个脚本在后面介绍。现在大家先弄清生成后的样子。对于一个软件包来讲，类型定义文件的位置通过 package.json 的 types 属性确定。

![img](images\74caf907d6af4d43b414a893e687cf3a~tplv-k3u1fbpfcp-zoom-1.jpg)
...
![img](images\b764efeffa64435e9b2d21434ac9fdef~tplv-k3u1fbpfcp-zoom-1.jpg)

具体写法为

![img](images\a1204b6735de43aa88dd241359f65e96~tplv-k3u1fbpfcp-zoom-1.jpg)
...
![img](images\087716ac2f7c446ca647e9a079b9b833~tplv-k3u1fbpfcp-zoom-1.jpg)
...
![img](images\bffe18884a054d7f900df9a572929a06~tplv-k3u1fbpfcp-zoom-1.jpg)
...
![img](images\523a0ca11b134bf5832549d780a4d6b1~tplv-k3u1fbpfcp-zoom-1.jpg)
