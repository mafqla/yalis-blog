<template>
  <div class="main-container-tag">
    ...
    <div>
      <!-- 标签云 -->
      <WordCloud :dataList="tagsList" :style="{ width: '100%', height: '130px' }" />
      <a-row :gutter="24">
        <!-- 标签列表区域 -->
        <a-col :span="24">
          <a-card :style="{ width: '100%', marginBottom: '20px' }">
            <a-tag @click="toggleTag(item.name)" v-for="item in tagsList" :class="selectTag === item.name
        ...
        <a-col :span="24">
          <a-list v-if="selectTag" :style="{ width: '100%' }">
            <template #header> 共 {{ articleList.length }} 篇文章 </template>
            <a-list-item v-for="(article, index) in articleList" :key="index">
              <div class="result-item">
                ...
                <!-- 文章元数据信息 -->
                <ArticleMetadata :article="article" :key="md5(article.date.string)" />
              </div>
            </a-list-item>
          </a-list>
...
//处理articleData转成tags，name为标签名，value为数量
...
// 点击指定Tag后进行选中
const selectTag = ref<string | null>(null)
const toggleTag = (tagTitle: string) => {
  selectTag.value = tagTitle
  //根据tagTitle获取文章列表
  const filteredArticles = articleData.filter(article => article.tags && article.tags.includes(tagTitle));
