...
const _imports_0 = "/assets/1.FtUGTNiW.image";
const _imports_1 = "/assets/2.jKbAi5NS.image";
const _imports_2 = "/assets/3.8-lDO4u2.image";
const _imports_3 = "/assets/4.wq75Y_7J.image";
const _imports_4 = "/assets/5.IZjMZr5K.image";
const _imports_5 = "/assets/6.iFw1PLlY.image";
const __pageData = JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/14-WebRTC 网关：项目实战以及插件使用.md","filePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/14-WebRTC 网关：项目实战以及插件使用.md","lastUpdated":1704161604000}');
const _sfc_main = { name: "knowledge/courses/WebRTC：实现私有化会议直播系统/14-WebRTC 网关：项目实战以及插件使用.md" };
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<div${ssrRenderAttrs(_attrs)}><p>上节我们了解了 <code>Janus</code>，接下来我们来看看如何将 <code>Janus</code> 应用到我们的实际项目中，并通过实战项目实现我们的第二种会议系统架构。</p><p>首先在项目中要使用 <code>Janus</code>，那必不可少的就是要引入对应的调用 API，如果大家翻过官网的话，不难发现在它的文档首页就有对应说明，直接了当地告诉大家 <code>JavaScript API</code>：</p><p><img${ssrRenderAttr("src", _imports_0)} alt="" loading="lazy"></p><p>上面截图第三行中有介绍怎么以 <code>module</code> 引入的，但是对于很多人来说太麻烦，接下来按照我说的步骤，我相信很容易将 <code>Janus</code> 引入项目并初始化看到效果。</p><h2 id="下载官方-js" tabindex="-1"><strong>下载官方</strong> <strong>JS</strong> <a class="header-anchor" href="#下载官方-js" aria-label="Permalink to &quot;**下载官方** **JS**&quot;">​</a></h2><p>官方 <a href="https://github.com/meetecho/janus-gateway/blob/master/html/janus.js" target="_blank" rel="noreferrer">JS 地址</a>，如果不下载，直接拷贝内容到自己创建的 JS 文件也可以，完成内容拷贝后，在这个文件最后加入一行代码，原因是 <code>Janus</code> 所有的操作包括初始化、销毁、加载插件等都是在官方 JS 文件中封装好的，可直接通过该变量控制，因此对外暴露后便于我们在项目中使用。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code"><code><span class="line"><span>// 对外暴露全局变量</span></span>
<span class="line"><span>export default Janus</span></span></code></pre></div><p><img${ssrRenderAttr("src", _imports_1)} alt="" loading="lazy"></p><p>看上图就是我创建的 <code>Janus API</code> 对应的 JS 文件。</p><p>有了 Janus API 文件，接下来给项目安装以下依赖，最新版本即可。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code"><code><span class="line"><span>&quot;webrtc-adapter&quot;: &quot;^8.2.0&quot;</span></span>
...
<span class="line"><span>            },</span></span>
<span class="line"><span>        });</span></span>
<span class="line"><span>},</span></span></code></pre></div><p><img${ssrRenderAttr("src", _imports_4)} alt="" loading="lazy"></p><p><img${ssrRenderAttr("src", _imports_5)} alt="" loading="lazy"></p><p>上面两个图，第一个就是客户端 <code>Janus</code> 初始化成功后并成功初始化 <code>VideoCall</code> 插件，第二个图为服务器上打印的日志，仔细看里面的几个类似时间戳的长数字，是不是一致？</p><p><strong>在这里要给大家一个温馨提示，每个客户端每次初始化成功都会返回一个 <code>sessionId</code> ，这个 ID 和服务器是互相关联的，如果你的服务器重启</strong> <strong>，</strong> <strong>那么当前客户端的 <code>sessionId</code> 也就没法用了。因此这个初始化过程一定要做全局可控。</strong></p><p>插件既然初始化了，那么怎么使用呢？关键就是我们在前面初始化代码中提到的句柄 <code>pluginHandle</code>，这个句柄可以操作所有的会话操作，比如呼叫、接听、挂断、媒体监控、媒体流控制等。</p><h2 id="课后题" tabindex="-1"><strong>课后题</strong> <a class="header-anchor" href="#课后题" aria-label="Permalink to &quot;**课后题**&quot;">​</a></h2><p>这节课的课后题就不扩展新的了，大家按照我说的从头下载 <code>Janus</code> 官方 JS，然后封装到自己的项目中，看看有没有什么问题，同时看看初始化过程中有无异常，会不会出现连接失败等现象。</p></div>`);
}
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("knowledge/courses/WebRTC：实现私有化会议直播系统/14-WebRTC 网关：项目实战以及插件使用.md");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const _14WebRTC______________ = /* @__PURE__ */ _export_sfc(_sfc_main, [["ssrRender", _sfc_ssrRender]]);
export {
  __pageData,
  _14WebRTC______________ as default
};
