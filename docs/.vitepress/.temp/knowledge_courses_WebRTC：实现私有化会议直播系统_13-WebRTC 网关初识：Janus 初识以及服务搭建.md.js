...
<span class="line"><span>janus-docker-janus-gateway-1  | HTTP transport timer started</span></span></code></pre></div><p><strong>温馨提示</strong></p><p>搭建过程中我们需要注意的几个点：</p><ol><li><code>rtp_port_range</code>：RTP 端口配置，<code>Janus</code> 作为 <code>WebRTC 的网关</code>，最离不开的就是和 “流” 打交道，WebRTC 实现的视频音频通话，涉及到的所有的媒体流发送和接收，都是 通过RTP 协议包。网关也一样，在每个客户端和网关服务器连接的过程中，这个流首先是要经过 <code>Janus</code> 的。因此，这个端口必须要放行，<code>Janus</code> 是自己配置了端口的，但是有些时候，我们不方便将该区间的端口开放，因此这个参数的目的就是让我们可以动态更改默认的 RTP 端口。</li><li><code>http/https</code>：这俩参数大家可以同时启用，也可以只用其中一个，我建议是只开放 Http 即可，要想使用 Https ，则可以通过该服务器指定的域名搭配 Nginx 代理映射即可，这样就无需单独为 <code>Janus</code> 维护一份 Https 证书。</li><li>我在配置中注释的配置文件路径，这些路径在老版本中都是放行且固定的，无需自己配置。但是新版本中这些路径是要自己手动打开且自行配置的。</li><li><strong><code>nat_1_1_mapping</code>：这个参数在内网映射公网时是用的到的，比如你的内部地址：192.168.101.1，要映射到公网 x.x.x.x，那么你就要配置这个参数。尤其你的<code>Janus</code>部署在公网，那么这个地址填写公网IP，当然一般私有化都是在内网的，可以不用处理</strong>。</li></ol><p>好了，这节课我们了解了<code>WebRTC</code>网关 <code>Janus</code>，同时也在服务器上搭建了该网关，下节课我们来学习如何将 Janus 用到我们的实际项目中。</p><h2 id="课后题" tabindex="-1">课后题 <a class="header-anchor" href="#课后题" aria-label="Permalink to &quot;课后题&quot;">​</a></h2><p>自己按照上面的步骤，完整部署一个 <code>Janus</code> 网关服务。</p></div>`);
}
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("knowledge/courses/WebRTC：实现私有化会议直播系统/13-WebRTC 网关初识：Janus 初识以及服务搭建.md");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const _13WebRTC______Janus_________ = /* @__PURE__ */ _export_sfc(_sfc_main, [["ssrRender", _sfc_ssrRender]]);
export {
  __pageData,
  _13WebRTC______Janus_________ as default
};
