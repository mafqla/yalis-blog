...
const _imports_0 = "/assets/1.Q5tNmb4C.image";
const _imports_1 = "/assets/2.cttRunnt.image";
const _imports_2 = "/assets/3.HN8QIIIK.image";
const _imports_3 = "/assets/4.UJWd-H7R.image";
const __pageData = JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/5-直播实战：WebRTC 实现类教师授课的 1 对 N 模式简易直播.md","filePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/5-直播实战：WebRTC 实现类教师授课的 1 对 N 模式简易直播.md","lastUpdated":1704161604000}');
const _sfc_main = { name: "knowledge/courses/WebRTC：实现私有化会议直播系统/5-直播实战：WebRTC 实现类教师授课的 1 对 N 模式简易直播.md" };
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<div${ssrRenderAttrs(_attrs)}><p>前面课程，我们完成<strong>一对一的视频通话</strong>后，大体上熟悉了<code>WebRTC</code>的基本用法以及它的会话流程。<code>WebRTC</code>的本质就是 P2P，即点对点的即时通讯，而这节课我们的目标是学会如何完成 1 对多的会话模式。</p><h2 id="基础构思" tabindex="-1">基础构思 <a class="header-anchor" href="#基础构思" aria-label="Permalink to &quot;基础构思&quot;">​</a></h2><p>在开始之前，我们先熟悉下这个最简单的讲课场景，首先看下图模拟场景，T 作为老师，需要将自己的画面实时地发送给下面的三个学生，但是学生却不需要将自己的画面同步给老师，而仅仅是在需要反馈的时候给予老师反馈即可。</p><p>说到这里，大家很明确这个场景实际就是一个小型直播，而我们这节课的目标也由此可以转变为“<code>WebRTC</code>实现简易直播系统”。</p><p><img${ssrRenderAttr("src", _imports_0)} alt="" loading="lazy"></p><p>场景和目标清晰了，接下来就是构思和实战了。</p><p><code>WebRTC</code>实现 P2P 视频通话以及 IM 大家都没问题，那么对于解决我们的目标而言都不成问题。T和 S-1 、S-2、S-3 单独完成视频通话和普通消息发送都没问题，那怎么实现一次性同时和三个学生建立通话呢？</p><p>很简单，老师 T 和他们三个单独建立视频通话后，将关联关系都保存在本地不就可以？</p><p>前面我们反复提到过<code>WebRTC</code>的核心就是<code>PeerConnection</code>对象，任何建立视频通话的双方都离不开这个对象，因为这里面包含连接双方的核心协商数据。所以只要 T 和三个学生建立关联关系时，都维护一份独立的<code>PeerConnection</code>对象即可。</p><p><img${ssrRenderAttr("src", _imports_1)} alt="" loading="lazy"></p><p>如上图，老师端保存三分独立的<code>PeerConnection</code>对象，而学生端只需要保存自己和老师的关联信息，即一份核心对象。用代码描述如下：</p><p><strong>老师端</strong></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code"><code><span class="line"><span>// T：9999 S-1：10001 S-2：10002 S-3:10003 分别代表上面流程图中的师生</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>客户端携带加入房间的基础信息，有基础用户信息和 pub 这个身份信息，服务端接受这个参数后，将其保存在房间缓存信息中：</p><p><img${ssrRenderAttr("src", _imports_2)} alt="" loading="lazy"></p><p><strong>老师 T</strong> <strong>端</strong>作为发布者，在加入房间后就需要发布自己的直播流，此时还没有任何学生和他建立连接。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code"><code><span class="line"><span>//?userId=9999&amp;roomId=10013&amp;nickname=S&amp;pub=pub</span></span>
...
<span class="line"><span>http://localhost:8080/demo03-one2many?userId=22222&amp;roomId=10012&amp;nickname=US02</span></span></code></pre></div><p><img${ssrRenderAttr("src", _imports_3)} alt="" loading="lazy"></p><ol start="3"><li>在演示过程中大家可能会看到弹幕和虚拟背景相关的，大家不必着急，这个是后面的内容，这节课的核心是搞懂直播这个概念以及<code>WebRTC</code>如何实现小型直播的。</li></ol><h2 id="完整代码仓库地址" tabindex="-1">完整代码仓库地址 <a class="header-anchor" href="#完整代码仓库地址" aria-label="Permalink to &quot;完整代码仓库地址&quot;">​</a></h2><p><a href="https://github.com/wangsrGit119/suke-webrtc-course/blob/main/webrtc-link-demo/src/views/demo03-one2many.vue" target="_blank" rel="noreferrer">本节课相关代码</a></p><h2 id="课后题" tabindex="-1">课后题 <a class="header-anchor" href="#课后题" aria-label="Permalink to &quot;课后题&quot;">​</a></h2><p>请大家先按照上述的过程完成本地的模拟场景，然后着重看下“<strong>学生S端</strong>”那里<code>pc.addTransceiver()</code>方法的作用（<a href="https://udn.realityripple.com/docs/Web/API/RTCRtpTransceiver/direction" target="_blank" rel="noreferrer">参考连接</a>），如果去掉这行代码，你的直播代码又会变成什么样子，还能正常进行直播吗？欢迎大家在留言区讨论。</p></div>`);
}
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("knowledge/courses/WebRTC：实现私有化会议直播系统/5-直播实战：WebRTC 实现类教师授课的 1 对 N 模式简易直播.md");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const _5______WebRTC__________1___N_______ = /* @__PURE__ */ _export_sfc(_sfc_main, [["ssrRender", _sfc_ssrRender]]);
export {
  __pageData,
  _5______WebRTC__________1___N_______ as default
};
