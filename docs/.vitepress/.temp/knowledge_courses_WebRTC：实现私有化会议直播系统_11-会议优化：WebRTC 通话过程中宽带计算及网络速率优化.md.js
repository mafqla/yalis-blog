...
<span class="line"><span>let total = biterate * 4 = 3.24 Mb/s</span></span></code></pre></div><p>上面的计算结果和在线的比特率计算器我们可以对比下：<a href="https://www.omnicalculator.com/other/streaming-bitrate" target="_blank" rel="noreferrer">在线比特率计算器</a></p><p><img${ssrRenderAttr("src", _imports_1)} alt="" loading="lazy"></p><p>总的算下来四个视频发出去需要消耗的上行宽带至少是 每秒 <strong>4M</strong> 左右，如果再加上你需要获取别的人员的视频，就需要单独的下行宽带也在 每秒 <strong>4M</strong> 左右，一般而言上行宽带和下行宽带是不等价的，从前面测试中就可以发现。</p><h2 id="优化会议质量" tabindex="-1">优化会议质量 <a class="header-anchor" href="#优化会议质量" aria-label="Permalink to &quot;优化会议质量&quot;">​</a></h2><p>了解了消耗宽带的计算，我们就需要优化我们的会议系统了，在<strong>小册第二讲</strong>中我们我们讲到了很多约束参数，根据实际情况我们再看下参数，实际情况帧速率 24 就可以了，所以我们设置 FPS 为 24，但是在网络状况差的时候是可以接受延缓的，因此如下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code"><code><span class="line"><span>const constraints = {</span></span>
...
<span class="line"><span>    }        </span></span>
<span class="line"><span>},</span></span></code></pre></div><ol start="5"><li>演示。请注意动图中的比特率变化。</li></ol><p><img${ssrRenderAttr("src", _imports_4)} alt="" loading="lazy"></p><h2 id="后话" tabindex="-1">后话 <a class="header-anchor" href="#后话" aria-label="Permalink to &quot;后话&quot;">​</a></h2><p>实际上，影响整个会议系统的不仅仅有宽带因素，还有自己当前主机的限制，每一路流都需要浏览器的编码解码，如果参会人数很多，你的笔记本性能很不好的话，那么就很容易造成卡顿，但是大多数情况下，比如二三十人的小型会议还是没有任何问题。</p><h2 id="课后题" tabindex="-1">课后题 <a class="header-anchor" href="#课后题" aria-label="Permalink to &quot;课后题&quot;">​</a></h2><p>这节课学完，我们的第一阶段算是完成了，后面就是利用开源流媒体服务器打造其他场景的会议系统了。希望在第一阶段结束后，大家都可以实现一个在区域网内可以使用的会议系统，加油！有任何问题留言即可。</p></div>`);
}
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("knowledge/courses/WebRTC：实现私有化会议直播系统/11-会议优化：WebRTC 通话过程中宽带计算及网络速率优化.md");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const _11______WebRTC_________________ = /* @__PURE__ */ _export_sfc(_sfc_main, [["ssrRender", _sfc_ssrRender]]);
export {
  __pageData,
  _11______WebRTC_________________ as default
};
