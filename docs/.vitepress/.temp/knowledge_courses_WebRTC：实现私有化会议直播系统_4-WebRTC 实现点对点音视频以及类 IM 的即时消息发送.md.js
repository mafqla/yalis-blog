...
<span class="line"><span>}</span></span></code></pre></div><ul><li>A 接受 B 的 <code>answer</code>信令后，将其设置为 <code>remoteDesc </code>。</li></ul><p><img${ssrRenderAttr("src", _imports_0)} alt="" loading="lazy"></p><blockquote><p><strong>注意看日志中的<code>candidate</code>，这个过程是贯穿整个会话的，直到<code>ice</code>候选完成。</strong></p></blockquote><p><strong>被呼叫端<code>B</code>。</strong></p><blockquote><p>被呼叫端的过程和呼叫端类似，大体代码如下：</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code"><code><span class="line"><span>async initCalleeInfo(localUid,fromUid){</span></span>
<span class="line"><span>        //初始化pc</span></span>
<span class="line"><span>        this.localRtcPc = new PeerConnection()</span></span>
...
<span class="line"><span>    }</span></span></code></pre></div><p>至此，所有的会话建立完成，在双方监听的 pc 核心方法<code>ontrack</code>中，就能拿到双方的音频和视频信息了，完整的代码我会放在文末，大家自取即可。不过纸上得来终觉浅，还需要你自己在实践中理解，这才是掌握最快速的。</p><p><img${ssrRenderAttr("src", _imports_1)} alt="" loading="lazy"></p><p>大家可以看下下图中具体的 <code>SDP</code>信息，实际上都是<code>WebRTC</code>封装好的，不需要我们去组装，如果你认真从前面看的，应该很容易理解，完成一个 P2P 的视频通话是不是很简单？</p><p><strong>不过我先打个预防针：在真正的复杂网络环境中</strong> <strong>，</strong> <strong>我们需要考虑的还有很多，如果之前大家了解过</strong> <strong><code>WebRTC</code></strong> <strong>相关的知识，一定对</strong> <strong>stun 和 turn</strong> <strong>这几个词不陌生，我们暂时不考虑这个，</strong> <strong>从最简单的网络环境中开始</strong> <strong>，完成我们的目标。</strong></p><p><img${ssrRenderAttr("src", _imports_2)} alt="" loading="lazy"></p><p><strong>通话过程中媒体流的变更</strong></p><p>完成以上视频通话，可能有人会问，怎么实现类似微信视频中，视频和音频之间随意切换，或摄像头前置后置切换呢？</p><p>这里我们就需要再学习一个知识点：<code>RTCRtpSender</code>对象。这个对象的接口支持变更你发送到对方的媒体，通过这个对象接口，你可以编辑更改流属性，从而达到控制远端媒体流的目的。</p><p>通过实例化后的<code>PeerConnection</code>对象调用<code>getSenders</code>方法，可获取每个媒体轨道对应<code>RTCRtpSender</code>对象。这里再解释下这个媒体轨道，我们在获取到媒体信息的时候，一般包含两部分，一部分音频信息(<code>audiotrack</code>)，一部分视频信息(<code>videotrack</code>)，因此这里的媒体轨道指的就是媒体信息。</p><ul><li>音频视频模式切换。</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code"><code><span class="line"><span>//获取发送到远端的具体媒体信息的发送方信息</span></span>
