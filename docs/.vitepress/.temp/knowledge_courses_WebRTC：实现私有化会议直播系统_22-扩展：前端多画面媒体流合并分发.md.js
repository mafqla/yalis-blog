...
const _imports_0 = "/assets/1.Hk7V0Zap.image";
const _imports_1 = "/assets/2.f6HMhAqW.image";
const _imports_2 = "/assets/3.tPxao1y5.image";
const _imports_3 = "/assets/4.qmlFDcCe.image";
const __pageData = JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/22-扩展：前端多画面媒体流合并分发.md","filePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/22-扩展：前端多画面媒体流合并分发.md","lastUpdated":1704161604000}');
const _sfc_main = { name: "knowledge/courses/WebRTC：实现私有化会议直播系统/22-扩展：前端多画面媒体流合并分发.md" };
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<div${ssrRenderAttrs(_attrs)}><p>上节课，我们提到了在<code>MCU</code>服务器处理音视频的过程中，可能会涉及到合并媒体流，实际上，这个合并媒体流的“任务”，在客户端我们也可以借助某些巧妙设计做到的，比如<code>Canvas</code>将两个媒体流通过“错位的”的方式贴合在一起，最终呈现出一个画面，但是画面是合成画面。</p><p>接下来，我们利用现成的开源组件库去实现这个目标。</p><h2 id="组件基本应用" tabindex="-1">组件基本应用 <a class="header-anchor" href="#组件基本应用" aria-label="Permalink to &quot;组件基本应用&quot;">​</a></h2><p>组件地址：<a href="https://github.com/t-mullen/video-stream-merger" target="_blank" rel="noreferrer">Github</a>。</p><p>演示地址：<a href="https://t-mullen.github.io/video-stream-merger/" target="_blank" rel="noreferrer">Demo</a>。</p><p>首先安装组件依赖：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code"><code><span class="line"><span>cnpm install video-stream-merger -S</span></span></code></pre></div><p><strong>实战</strong></p><p>比如将一个 MP4 视频和摄像头的画面合成。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code"><code><span class="line"><span>//组件引入</span></span>
...
<span class="line"><span>       </span></span>
<span class="line"><span>},</span></span></code></pre></div><p><img${ssrRenderAttr("src", _imports_1)} alt="" loading="lazy"></p><h2 id="屏幕分享-摄像头合成推流" tabindex="-1">屏幕分享+摄像头合成推流 <a class="header-anchor" href="#屏幕分享-摄像头合成推流" aria-label="Permalink to &quot;屏幕分享+摄像头合成推流&quot;">​</a></h2><p>在以往视频会话过程中，如果遇到屏幕分享，自己的摄像头画面不会展示，这也是大多数会议系统常规的做法。但是，通过我们这节课学到的知识，就可以直接将屏幕分享流和摄像头流合并然后作为一个流分发，让对方不仅仅看到你的屏幕画面，同时能看到你的摄像头画面。</p><p>好了，接下来我们来看看怎么将两个动态媒体流组合起来。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code"><code><span class="line"><span>//屏幕分享和摄像头</span></span>
...
<span class="line"><span>  },</span></span></code></pre></div><p><strong>效果如下</strong></p><p><img${ssrRenderAttr("src", _imports_2)} alt="" loading="lazy"></p><p>合成的参数都是可以自己调的，比如位置在左上角或者右上角，自己按照实际情况来即可，设置的参数就是添加<code>Stream</code>的时候指定即可。</p><p>以上的场景我们可以再具体下，当屏幕分享完成之后，想要关闭分享画面（<code>关闭分享按钮在开始分享后下面会显示，注意动图中按钮的变化</code>），仅仅保留摄像头画面，怎么实现呢？实际上这个组件也是可以自动适配的，选择关闭后，对方的画面就只剩下一个了，但是请注意此时剩下那个画面的位置是有问题的，比如下面这样：</p><p><img${ssrRenderAttr("src", _imports_3)} alt="" loading="lazy"></p><p>虽然对方看到的画面没有问题，但是我们这边显示的是有问题的，看着很别扭。因此我们可以按照常规做法，按照</p><p>我们前面反复提到的媒体控制（<a href="https://juejin.cn/book/7168418382318927880/section/7172837736468971551" target="_blank" rel="noreferrer">《10|会议实战：实时通话过程中音频、视频画面实时控制切换》</a>），去直接替换<code>RTCSender</code>中的发布流即可。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code"><code><span class="line"><span>//直接置换发送器中的媒体流（）</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><h2 id="总结" tabindex="-1">总结 <a class="header-anchor" href="#总结" aria-label="Permalink to &quot;总结&quot;">​</a></h2><p>通过这种方式可以解决多流传输而带来的宽带资源消耗的问题，同时也将服务端的“合流”能力以一种很巧妙的方式转移到了客户端，实际上，对客户端而言，也并没有消耗多少资源，因为本质上还是用的<code>Canvas</code>画布，然后画布转换成流。</p><p>这样即使我们客户端有 N 多个画面：摄像头、静态视频播放、多个屏幕共享流，或者纯音频等都是可以合并到一个流中，如果将此功能在自己的业务中扩展，就可以实现一个画面编辑器，通过此在线编辑器组合任意流然后分发。</p><p>上面提到的在线编辑器像不像直播用到的<code>直播姬</code>？将各种小的组件组合起来，然后作为直播的合成画面发布，观众看到的也是合成的，而且对于资源的占用也并不是很大。</p><h2 id="相关源码" tabindex="-1">相关源码 <a class="header-anchor" href="#相关源码" aria-label="Permalink to &quot;相关源码&quot;">​</a></h2><p><a href="https://github.com/wangsrGit119/suke-webrtc-course/blob/main/webrtc-link-demo/src/views/stream-merger-push.vue" target="_blank" rel="noreferrer">本节相关源码</a></p><h2 id="课后题" tabindex="-1">课后题 <a class="header-anchor" href="#课后题" aria-label="Permalink to &quot;课后题&quot;">​</a></h2><p>学完本节课，希望大家可以对前面学到的所有内容做个归纳，梳理一份自己会议系统相关的思维导图，从基础的<code>WebRTC</code>会话流程到怎么实现会议等等。</p></div>`);
}
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("knowledge/courses/WebRTC：实现私有化会议直播系统/22-扩展：前端多画面媒体流合并分发.md");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const _22________________ = /* @__PURE__ */ _export_sfc(_sfc_main, [["ssrRender", _sfc_ssrRender]]);
export {
  __pageData,
  _22________________ as default
};
