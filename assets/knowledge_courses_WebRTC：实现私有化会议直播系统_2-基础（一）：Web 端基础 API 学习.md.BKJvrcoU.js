...
<span class="line"><span>            })</span></span>
<span class="line"><span>            .catch(handleError);</span></span>
<span class="line"><span>    }</span></span></code></pre></div><p>这个代码片段的主要作用就是获取用户设备上所有的摄像头和麦克风信息，起关键作用的是<code>enumerateDevices</code>函数，但是在调用这个关键函数之前，<code>getUserMedia</code>函数出现在了这里，它的出现是用户在访问服务时直接调用用户摄像头，此时如果用户授权且同意使用设备摄像头、麦克风，那么<code>enumerateDevices</code>函数就能获取设备信息了，在这里<code>getUserMedia</code>函数可以理解为获取摄像头或者麦克风权限集合的<strong>探路函数</strong>。</p><p>看下图，我将我电脑上使用<code>enumerateDevices</code>函数加载到的信息，根据前面提到的字段<code>kind</code>，将其分三类并打印到控制台。</p><p><img src="`+o+`" alt="" loading="lazy"></p><p>千万不要小看现在获取到的这些信息哦，在后面视频通话或会议过程中，我们需要抉择摄像头用前置还是后置，麦克风是用蓝牙还是有线，都是离不开这些信息的。</p><p>在拿到所有的摄像头麦克风信息之后，我们需选出最终要参与视频通话的那个信息体，看上图中 <strong><code>VideoIn</code><strong>数组里面</strong><code>label:&quot;eseSoft Vcam&quot;</code></strong> <strong>，</strong> 这个摄像头就是我想要参会的摄像头，那么我怎样指定让代码去选择这个摄像头呢？这里就涉及到了<code>getUserMedia</code>的约束参数<code>constraints</code> 。</p><h3 id="媒体约束-constraints" tabindex="-1">媒体约束 constraints <a class="header-anchor" href="#媒体约束-constraints" aria-label="Permalink to &quot;媒体约束 constraints&quot;">​</a></h3><p>在具体讲解约束参数 constraints 之前，大家先看下面这段示例代码。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>let constraints = {video:true, audio: true} </span></span>
<span class="line"><span>--</span></span>
...
<span class="line"><span>})</span></span></code></pre></div><ol><li><strong>audio</strong>为<strong>true</strong></li></ol><p><img src="`+t+'" alt="" loading="lazy"></p><ol start="2"><li><strong>audio</strong> 为 <strong>false</strong></li></ol><p><img src="'+l+`" alt="" loading="lazy"></p><p>请留意上面两图的对比，当去掉音频后，第二张图少了个勾选系统音频的 radio 框。</p><h3 id="完整案例" tabindex="-1"><strong>完整案例</strong> <a class="header-anchor" href="#完整案例" aria-label="Permalink to &quot;**完整案例**&quot;">​</a></h3><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>/**</span></span>
...
<span class="line"><span>               track.stop();</span></span>
<span class="line"><span>           });</span></span>
<span class="line"><span>       }</span></span></code></pre></div><p>如果不销毁，你可以看到在标签页旁边一直有个小红圈闪烁，鼠标按上去提示正在使用当前设备的摄像头，因此在后面的开发中保持好习惯：结束自己会议后或页面用完摄像头后，一般除了强制刷新，也可以调用上面代码清除正在使用的<code>stream</code>调用。</p><p>好了，这节课我们我们掌握了两个最重要的 API，下节课我们开始搭建一个信令服务器，同时完成 <code>P2P</code> (单人对单人)的视频通话（跑代码的时候一定要记得前面提到的<strong>安全源</strong>哦）。</p><h2 id="课后题" tabindex="-1">课后题 <a class="header-anchor" href="#课后题" aria-label="Permalink to &quot;课后题&quot;">​</a></h2><p>大家可以思考思考，在分辨率和<code>FPS</code>的配置以及宽带的利用上还有有哪些场景和实践，欢迎在留言区讨论。</p>`,86),h=[g];function u(m,v,b,k,f,_){return n(),a("div",null,h)}const y=s(d,[["render",u]]);export{I as __pageData,y as default};
