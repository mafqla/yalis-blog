import{_ as n,o as a,c as e,a4 as p}from"./chunks/framework.CW-6O5Zu.js";const l="/assets/1.k91g4KeF.image",g=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/12-网络配置：Coturn 网络穿透服务器的搭建及使用.md","filePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/12-网络配置：Coturn 网络穿透服务器的搭建及使用.md","lastUpdated":1776396922000}'),t={name:"knowledge/courses/WebRTC：实现私有化会议直播系统/12-网络配置：Coturn 网络穿透服务器的搭建及使用.md"};function i(r,s,o,c,d,u){return a(),e("div",null,[...s[0]||(s[0]=[p(`<p>这节课我们来说说 Coturn 网络打洞服务器， Coturn 是一个开源的 TURN &amp; STUN 服务器，<a href="https://github.com/coturn/coturn" target="_blank" rel="noreferrer">Github地址</a>。</p><p>这里又扯到了 TURN 和 STUN 两个特别的东西，在具体介绍这两个名词的知识前，我们先看看在用 <code>WebRTC</code>进行视频通话过程中会遇到什么问题。</p><p>需要知道的是， <code>WebRTC</code>进行通话过程中是没有任何视频服务器的，所有的 RTP 数据包都是直接通过双方的 IP 和对应的端口进行发送和传输的（大家感兴趣的话，可以尝试用 GB28181 和监控摄像头交互然后获取监控的 RTP 包 ）。</p><p>因此通常在开始通话之前，就必须进行 ICE 协商获取双方的 IP 以及端口，如果通话是区域网则简单，直接区域网 IP 即可，但通话如果是在公共复杂网络，且客户端并未直接接入公共网路，而是仅接入到本地 <code>NAT 网关</code>中呢？</p><p>此时是无法直接获取通话客户端的公有 IP 的。这个时候就需要特定的东西去<strong>获取客户端真实且互相可以抵达的公有 IP</strong>，而 STUN 服务器的作用就是干这个的，有了它就可以让通话双方发现对方的公共通讯地址。</p><p>接上面，能发现公有地址不代表就能进行通话，进行通话的前提是允许客户端能够在相应端口进行上行和下行数据的发送和接收。就好比你电脑启动了一个服务但是防火墙是开着的，那么别人访问你的服务还是没法访问的。此时 TURN 来了，<strong>它的作用就是充当双方中继地址，将双方的流量中继到当前服务器从而实现双方流量的交换</strong>。</p><h2 id="打洞服务器的搭建" tabindex="-1"><strong>打洞服务器的搭建</strong> <a class="header-anchor" href="#打洞服务器的搭建" aria-label="Permalink to &quot;**打洞服务器的搭建**&quot;">​</a></h2><p>为了让大家免于各种复杂环境的困扰，我们使用容器来搭建。</p><ol><li>首先提前下载对应的配置文件：<a href="https://github.com/coturn/coturn/blob/master/docker/coturn/turnserver.conf" target="_blank" rel="noreferrer">官方文件</a>。</li></ol><ol start="2"><li>修改重要的配置，<strong>注意不是替换，是找到下面配置的地址</strong>，注意看每个配置的描述。</li></ol><blockquote><p>在纯内网环境，很多时候服务器只有一个 IP ，因此下面遇到的 IP 配置都配置同一个，而 realm 使用默认即可，无需配置。</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>#中继服务器监听的IP地址，可以指定多个IP  最好直接写 ip addr  输出的etho的ip</span></span>
...
<span class="line"><span>log-file=stdout # 注释 sys-log</span></span></code></pre></div><ol start="3"><li>挂载配置文件位置启动容器。</li></ol><blockquote><p>Coturn 我这里指定的镜像为 4.6 版本的，如果有新的，大家可以尝试最新稳定版本。</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>sudo docker run -d --privileged=true --name=mycoturn   --network=host \\</span></span>
<span class="line"><span>           -v /home/ubuntu/turnserver/turnserver.conf:/my/turnserver.conf \\</span></span>
<span class="line"><span>       coturn/coturn:4.6 -c /my/turnserver.conf</span></span></code></pre></div><ol start="4"><li>日志。</li></ol><blockquote><p>下面的 167.160.189.179 IP 为我搭建的启动成功后的日志。</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>root@C20220407114397:~# docker logs -f mycoturn</span></span>
...
<span class="line"><span>0: : Prometheus collector disabled, not started.</span></span></code></pre></div><ol start="5"><li>测试是否成功。</li></ol><p>最简单的方法是直接在浏览器输入服务器 <code>IP:3478</code>，然后看输出日志，如果输出以下类似日志，则大概率部署成功。</p><blockquote><p>可以注意到日志中 username 的值。</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>86396: : session 001000000000000001: usage: realm=&lt;example.org&gt;, username=&lt;&gt;, rp=1, rb=20, sp=1, sb=88</span></span>
...
<span class="line"><span>98891: : session 001000000000000002: closed (2nd stage), user &lt;&gt; realm &lt;example.org&gt; origin &lt;&gt;, local 167.16</span></span></code></pre></div><h2 id="项目中使用" tabindex="-1"><strong>项目中使用</strong> <a class="header-anchor" href="#项目中使用" aria-label="Permalink to &quot;**项目中使用**&quot;">​</a></h2><ol><li>代码配置</li></ol><p><strong>iceTransportPolicy</strong> 配置表示所有配置此参数的客户端在通话的时候都走 Turn 服务器中继。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>//全局变量</span></span>
...
<span class="line"><span> let pc = new PeerConnection(that.rtcPcParams)</span></span></code></pre></div><ol start="2"><li>中继日志</li></ol><blockquote><p>注意看 username 字段以及后面的 incoming packet。</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>519121: : session 000000000000000016: delete: realm=&lt;example.org&gt;, username=&lt;suc&gt;</span></span>
