...
<span class="line"><span>let total = biterate * 4 = 3.24 Mb/s</span></span></code></pre></div><p>上面的计算结果和在线的比特率计算器我们可以对比下：<a href="https://www.omnicalculator.com/other/streaming-bitrate" target="_blank" rel="noreferrer">在线比特率计算器</a></p><p><img src="`+l+`" alt="" loading="lazy"></p><p>总的算下来四个视频发出去需要消耗的上行宽带至少是 每秒 <strong>4M</strong> 左右，如果再加上你需要获取别的人员的视频，就需要单独的下行宽带也在 每秒 <strong>4M</strong> 左右，一般而言上行宽带和下行宽带是不等价的，从前面测试中就可以发现。</p><h2 id="优化会议质量" tabindex="-1">优化会议质量 <a class="header-anchor" href="#优化会议质量" aria-label="Permalink to &quot;优化会议质量&quot;">​</a></h2><p>了解了消耗宽带的计算，我们就需要优化我们的会议系统了，在<strong>小册第二讲</strong>中我们我们讲到了很多约束参数，根据实际情况我们再看下参数，实际情况帧速率 24 就可以了，所以我们设置 FPS 为 24，但是在网络状况差的时候是可以接受延缓的，因此如下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>const constraints = {</span></span>
...
<span class="line"><span>};</span></span></code></pre></div><p>而在很多情况下，多人会议并不需要去关注画面的清晰度，此时可以将整体的分辨率调低到 <code>480P</code> 左右，当然 <code>360P</code> 也是可以接受的，如下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>const constraints = {</span></span>
<span class="line"><span>    audio: true,</span></span>
<span class="line"><span>    video: {</span></span>
...
<span class="line"><span>        frameRate: { min:10 , ideal: 15, max: 24 }</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>};</span></span></code></pre></div><p>需要注意的是，遇到屏幕分享这个场景，大家记得一定要调高分辨率，不然对面看到的画面就是马赛克了。</p><p>除了上述设置限制<code>分辨率</code>和<code>FPS</code>之外，我们还可以限制发送的 <code>Bitrate</code>即自己的出口宽带。在和另一端建立<code> RTC 连接</code>之后，我们可以根据上一讲中提到的画面控制参数来变更发送的 <code>Bitrate</code>。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>//pc 为 核心关联 PeerConnection实例 </span></span>
...
<span class="line"><span> await sender.setParameters(params);</span></span></code></pre></div><h2 id="会议实时宽带计算" tabindex="-1">会议实时宽带计算 <a class="header-anchor" href="#会议实时宽带计算" aria-label="Permalink to &quot;会议实时宽带计算&quot;">​</a></h2><p>在<a href="https://juejin.cn/book/7168418382318927880/section/7172208545868283917?scrollMenuIndex=1" target="_blank" rel="noreferrer">《09 | 会议实战：WebRTC 实现多房间多用户的第一种架构会议系统》</a>这节课的会议 Demo 中，大家点击统计按钮就会在参会人员画面中显示实时消耗的宽带，计算的核心还是我们反反复复提到了多次的<code>PeerConnection</code> <strong>实例对象</strong>，下面代码就是计算所有和当前客户端建立<code> RTC 连接</code>后实时视频过程中宽带统计的计算方式。</p><blockquote><p>注意代码中的注释：出口宽带和入口宽带，出口宽带计算的是你发送流单位时间所消耗的比特流，而入口宽带就是通过当前<code>RTC连接</code>远程客户端发送的媒体流到你的客户端展示所消耗的比特流。</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>calculateSendBitrate(userId,pc){</span></span>
...
<span class="line"><span>        })</span></span>
<span class="line"><span>},</span></span></code></pre></div><p>核心还是通过<code>RTCPeerConnection</code>对象的<code>getStats</code>方法计算，定时器中定时去获取这个方法返回的信息，看上面代码中<code>report.type === &#39;outbound-rtp&#39;</code>出口宽带这里。</p><h2 id="项目操作演示" tabindex="-1">项目操作演示 <a class="header-anchor" href="#项目操作演示" aria-label="Permalink to &quot;项目操作演示&quot;">​</a></h2><ol><li>打开项目，找到模块：多对多网络视频。</li></ol><p><img src="`+o+'" alt="" loading="lazy"></p><ol start="2"><li>用不同账户入会，两个用户即可。</li></ol><ol start="3"><li>打开视频后第二个人的页面下面点击 <code>stats</code>按钮。稍等片刻，就是实时显示对面用户画面的实时比特率了。</li></ol><p><img src="'+i+`" alt="" loading="lazy"></p><ol start="4"><li>切换第一个用户标签页面，点击对面用户视频画面，此时触发画面控制参数设置。</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>async setBiterate(uid){</span></span>
