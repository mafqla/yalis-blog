...
<span class="line"><span>        const send = senders.find((s) =&gt; s.track.kind === &#39;audio&#39;)</span></span>
<span class="line"><span>        send.track.enabled = b</span></span>
<span class="line"><span>})</span></span></code></pre></div><p><strong>视频暂停恢复</strong></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>//单个RTC关联视频</span></span>
<span class="line"><span>const senders = localRTC .getSenders();</span></span>
...
<span class="line"><span>    const send = senders.find((s) =&gt; s.track.kind === &#39;audio&#39;)</span></span>
<span class="line"><span>    send.replaceTrack(audioTrack)</span></span>
<span class="line"><span>})</span></span></code></pre></div><p><strong>视频切换</strong></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>const [videoTrack] = stream.getVideoTracks();</span></span>
<span class="line"><span>//单个RTC关联</span></span>
<span class="line"><span>const senders = localRTC.getSenders();</span></span>
<span class="line"><span>const send = senders.find((s) =&gt; s.track.kind === &#39;video&#39;)</span></span>
...
<span class="line"><span>  await sender.setParameters(params);</span></span></code></pre></div><ol start="2"><li>通过强制 Constraints 设置。</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>//过滤视频</span></span>
...
<span class="line"><span>await sender.track.applyConstraints({ height ,frameRate,aspectRatio});</span></span></code></pre></div><p>这里我们需要注意：</p><ol><li>通过前面第一种方式设置参数，在很多浏览器会被阻止，因此建议用第二种方式，直接强制约束来设置通用参数。</li></ol><ol start="2"><li>在设置某些参数之前，我们需要知道，摄像头是否支持这个参数，假如你的摄像头本身的分辨率只有 <code>720X640</code> ，那么你设置<code>1920X1080</code>是不会生效的哦。</li></ol><h2 id="项目操作指南" tabindex="-1">项目操作指南 <a class="header-anchor" href="#项目操作指南" aria-label="Permalink to &quot;项目操作指南&quot;">​</a></h2><ol><li>打开项目，找到模块：多人视频聊天。</li></ol><ol start="2"><li>不同客户端 A、B 加入会议。</li></ol><ol start="3"><li>设置我们想要变更的参数</li></ol><p>这里我设置的参数有三个：<code>height</code>（帧高度） 、<code>frameRate</code>（FPS） 、<code>aspectRatio</code>（分辨率宽高之比） 。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>async getRtcPeerInfo(uid){</span></span>
...
<span class="line"><span>                </span></span>
<span class="line"><span>        }</span></span>
<span class="line"><span>},</span></span></code></pre></div><ol start="4"><li>操作①：点击对方画面框框（Video DOM 元素），然后观察控制台和右下角画面显示。</li></ol><ol start="5"><li>操作②：点击自己的视频（右下角显示画面），观察控制台。</li></ol><p><img src="`+o+'" alt="" loading="lazy"></p><ol start="6"><li>参数变更说明：上图红色圈圈中的就是设置前的参数，后面绿色的就是变更后的参数，大家可以发现，这个和我代码中的一致。请注意：我在代码中并没有设置分辨率的 <code>width</code>，但是它也变了，为何？答案就在本节前面参数详解第四条中：当你设置固定的宽高之后<code>aspectRatio</code>会自动变更，相反，仅设置<code>aspectRatio</code>参数后，其他 <code>height</code> 和 <code>width</code> 参数也会变动。</li></ol><h2 id="代码仓库" tabindex="-1">代码仓库 <a class="header-anchor" href="#代码仓库" aria-label="Permalink to &quot;代码仓库&quot;">​</a></h2><p><a href="https://github.com/wangsrGit119/suke-webrtc-course/blob/main/webrtc-link-demo/src/views/demo03-many2many.vue" target="_blank" rel="noreferrer">本节课代码位置</a></p><h2 id="课后题" tabindex="-1">课后题 <a class="header-anchor" href="#课后题" aria-label="Permalink to &quot;课后题&quot;">​</a></h2><ol><li>请大家根据这节课学到的内容，去改造下点对点视频通话过程中媒体流的变更。</li><li>完成第一个问题后，可以尝试摄像头和屏幕分享流的切换，即和对方分享自己的桌面。</li></ol>',60)])])}const b=a(i,[["render",c]]);export{u as __pageData,b as default};
