...
<span class="line"><span>                }</span></span>
<span class="line"><span>            });</span></span>
<span class="line"><span>},</span></span></code></pre></div><h2 id="会议插件基本功能" tabindex="-1"><strong>会议插件基本功能</strong> <a class="header-anchor" href="#会议插件基本功能" aria-label="Permalink to &quot;**会议插件基本功能**&quot;">​</a></h2><p>初始化完成之后就是使用了，而使用的具体细节就需要看看 <code>videoRoom</code>插件的具体功能了，下面我们来看看插件的基本功能和基础用法。</p><p><strong>动态创建会议室</strong>。要实现会议，前提条件就是可以动态提供不同的房间让不同的客户端去加入，<code>videoRoom</code>插件提供了对应创建房间的 API ，具体参数看下面代码注释：</p><p><img src="`+i+`" alt="" loading="lazy"></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>createJanusRoom(roomId,roomUserCount,bitrate,pin,desc){</span></span>
<span class="line"><span>      const that = this;</span></span>
...
<span class="line"><span>        }</span></span>
<span class="line"><span>      })</span></span>
<span class="line"><span>    },</span></span></code></pre></div><p><strong>房间用户获取和控制</strong>。有了会议室，正常操作就是实时获取房间用户列表，并控制对应房间用户，比如踢出、静音等操作。</p><p><img src="`+o+`" alt="" loading="lazy"></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>//房间用户列表加载</span></span>
<span class="line"><span>videoRoomPluginHandle.send({</span></span>
...
<span class="line"><span>          }</span></span>
<span class="line"><span>  })</span></span>
<span class="line"><span>},</span></span></code></pre></div><p><strong>监听服务器响应的事件</strong>。在和网关服务器完成 P2P 连接后，其他用户加入的房间如果<code>发布或暂停</code>媒体信息网关服务器则会通知先前加入的用户，以让其他用户同步做出变更。</p><p><img src="`+r+`" alt="" loading="lazy"></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>onMessageForVideoRoom(msg,jsep){</span></span>
<span class="line"><span>            const that = this</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  });</span></span>
<span class="line"><span>},</span></span></code></pre></div><p>好了，以上就是利用 videoRoom 插件的巧妙设计（媒体发布和订阅交集）实现 SFU 架构会议系统的基本步骤，整个流程很简单，需要大家要注意的点也就是其中的媒体发布和媒体订阅代码逻辑。</p><h2 id="项目操作指南" tabindex="-1">项目操作指南 <a class="header-anchor" href="#项目操作指南" aria-label="Permalink to &quot;项目操作指南&quot;">​</a></h2><ol><li>打开项目，打开下面模块：Janus 会议室。</li></ol><p><img src="`+u+'" alt="" loading="lazy"></p><ol start="2"><li>先创建房间，然后点击加入房间。创建房间只需第一次访问的时候创建即可，之后会在网关服务器内存中保存，如果想要持久化，则可以看前面创建房间的配置。</li></ol><p><img src="'+d+'" alt="" loading="lazy"></p><ol start="3"><li>用户 ID 不用更改，默认使用时间戳；修改用户名让不同用户加入到同一个房间。</li></ol><p><img src="'+g+'" alt="" loading="lazy"></p><p><img src="'+m+'" alt="" loading="lazy"></p><h2 id="代码仓库" tabindex="-1"><strong>代码仓库</strong> <a class="header-anchor" href="#代码仓库" aria-label="Permalink to &quot;**代码仓库**&quot;">​</a></h2><p><a href="https://github.com/wangsrGit119/suke-webrtc-course/blob/main/webrtc-link-demo/src/views/demo04-janus-02.vue" target="_blank" rel="noreferrer">本接口代码仓库地址</a></p><h2 id="课后题" tabindex="-1"><strong>课后题</strong> <a class="header-anchor" href="#课后题" aria-label="Permalink to &quot;**课后题**&quot;">​</a></h2><p>这节课我并没有将除了会议的其他功能写在 DEMO 中，课后大家在实现会议功能之后，尝试将踢人、麦克风控制、视频控制等操作加在上面，看看有没有难度。</p>',60),q=[b];function v(_,f,k,C,y,P){return a(),n("div",null,q)}const T=s(h,[["render",v]]);export{j as __pageData,T as default};
