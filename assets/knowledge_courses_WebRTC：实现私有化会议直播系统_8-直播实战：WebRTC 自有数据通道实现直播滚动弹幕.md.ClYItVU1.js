import{_ as n,o as s,c as e,a4 as p}from"./chunks/framework.CW-6O5Zu.js";const l="/assets/1.FbJ50dRs.image",t="/assets/2.Cv4W7Q_p.image",o="/assets/3.Cv6goi7V.image",b=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/8-直播实战：WebRTC 自有数据通道实现直播滚动弹幕.md","filePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/8-直播实战：WebRTC 自有数据通道实现直播滚动弹幕.md","lastUpdated":1776407975000}'),c={name:"knowledge/courses/WebRTC：实现私有化会议直播系统/8-直播实战：WebRTC 自有数据通道实现直播滚动弹幕.md"};function i(r,a,d,h,m,u){return s(),e("div",null,[...a[0]||(a[0]=[p(`<p>从点对点视频通话到点到多的小型直播间，再到直播间虚拟背景的切换，如果认真消化完这些内容，那么我相信大多数人已经从小白正式入门<code>WebRTC</code>了。接下来这节课我们就先不讲<code>WebRTC</code>会议模式，我们先玩个好玩的，如题。</p><p>一般意义上要完成弹幕，首选必须是即时通讯，比如大家熟知的 Websocket，通过即时消息实现实时弹幕展示。但是我们在前面<a href="https://juejin.cn/book/7168418382318927880/section/7172117235333824520" target="_blank" rel="noreferrer">《04 | 实现点对点音视频及类 IM 的即时消息发送》</a>那节课中提到了，利用<code>WebRTC</code>自己的实时数据传输通道即可实现类似 IM 的即时通讯功能，因此我们就用<code>WebRTC</code>自带的功能去完成“直播+滚动弹幕”。</p><p>正式开始之前，大家可以先翻到前面课程再熟悉下，我们接下来就直接进入主题。</p><h2 id="基础流程" tabindex="-1"><strong>基础流程</strong> <a class="header-anchor" href="#基础流程" aria-label="Permalink to &quot;**基础流程**&quot;">​</a></h2><ol><li>建立<code>WebRTC</code>会话，并在初始化监听阶段创建数据通道（<code>DataChannel</code>）。</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>/**</span></span>
...
<span class="line"><span>await this.createDataChannel(initPc,localUid,remoteUid)</span></span></code></pre></div><ol start="2"><li>通道建立成功，如果是 A 和 B 通话，那么 A 创建通道成功后，B 端则可以看到如下信息：</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>pc.ondatachannel = function(ev) {</span></span>
...
<span class="line"><span>    ......</span></span></code></pre></div><p><strong>主播端：</strong> 请注意，观众每次加进来都会和主播建立一次<code>WebRTC</code>关联，每次都会创建主播和该观众的数据通道并保存在本地<strong>数据通道 Map</strong> 中，如此主播在发送弹幕的时候，遍历拿到所有观众的数据通道，再通过该通道将消息广播，观众端即可看到主播的消息。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>async onRemoteOffer(fromUid,offer){</span></span>
...
<span class="line"><span>},</span></span></code></pre></div><h2 id="弹幕公共化" tabindex="-1">弹幕公共化 <a class="header-anchor" href="#弹幕公共化" aria-label="Permalink to &quot;弹幕公共化&quot;">​</a></h2><p>看完上面的流程和代码，有细心的同学可能会发现，观众端只和主播建立了<code>WebRTC</code>关联，并没有和其他的观众建立关联，那么，怎么能看到其他观众发送的弹幕呢？</p><p>这就要提到主播端维护的<strong>数据通道 Map</strong>了，利用这个 Map 中的通道，主播端可以将任何消息私信发给任何观众，因此当有观众发送弹幕到主播端的时候，主播利用该 Map 中的所有数据通道，将消息遍历发送给每个通道，即可实现弹幕的广播。</p><h2 id="弹幕组件" tabindex="-1">弹幕组件 <a class="header-anchor" href="#弹幕组件" aria-label="Permalink to &quot;弹幕组件&quot;">​</a></h2><p>要实现弹幕，必不可少的就是弹幕组件了。本小册用到的组件如下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>&quot;danmaku&quot;: &quot;^2.0.4&quot;,</span><span> // npm install danmaku 即可安装</span></span></code></pre></div><p>具体用法如下，注意看代码中的注释，弹幕组件和媒体播放容器绑定，因此在主播端和观众端需要区分，参数有颜色设置、速度设置，以及字体设置等等，具体大家可以看<a href="https://github.com/weizhenye/Danmaku" target="_blank" rel="noreferrer">官方文档</a>。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>/**</span></span>
...
<span class="line"><span>http://localhost:8080/demo03-one2many?userId=22222&amp;roomId=10012&amp;nickname=US02</span></span></code></pre></div><ol start="4"><li>操作：主播端在页面下方输入框输入消息，然后点击发送，注意观察 <code>US01</code> 和 <code>US02</code> 两端的<code>控制台</code>以及<code>弹幕</code>显示。</li></ol><ol start="5"><li>效果。</li></ol><p><img src="`+t+'" alt="" loading="lazy"></p><p><strong>温馨提示</strong></p><p>弹幕我是利用组件展示在 <code>Video 元素</code>之上，大家实际用的时候可能会自定义其他的展示窗体，因此你需要明白的是数据通道数据过来的代码位置，在这里你可以<code>自定义数据操作逻辑</code>。</p><p><img src="'+o+'" alt="" loading="lazy"></p><h2 id="课后题" tabindex="-1"><strong>课后题</strong> <a class="header-anchor" href="#课后题" aria-label="Permalink to &quot;**课后题**&quot;">​</a></h2><p>这节课算是把<code>WebRTC</code>的数据通道掌握了，实际上数据通道不仅仅可以实现弹幕，利用这个，还可以实现很多花样，课后大家可以再熟悉熟悉数据通道用法，<strong>传输文件、图片</strong> 等数据。</p>',44)])])}const k=n(c,[["render",i]]);export{b as __pageData,k as default};
