import{_ as s,o as n,c as e,a4 as p}from"./chunks/framework.CW-6O5Zu.js";const o="/assets/1.Z0azDDvu.image",l="/assets/2.BxUUEfPR.image",c="/assets/3.CprDlyoP.image",t="/assets/4.CzFZ2fAn.image",i="/assets/5.CAinSJed.image",d="/assets/6.t1T3spIl.image",r="/assets/7.DMUhEU8E.image",h="/assets/8.DLy9Rcpf.image",_=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/4-WebRTC 实现点对点音视频以及类 IM 的即时消息发送.md","filePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/4-WebRTC 实现点对点音视频以及类 IM 的即时消息发送.md","lastUpdated":1776395729000}'),u={name:"knowledge/courses/WebRTC：实现私有化会议直播系统/4-WebRTC 实现点对点音视频以及类 IM 的即时消息发送.md"};function m(g,a,k,b,f,v){return n(),e("div",null,[...a[0]||(a[0]=[p(`<p>前面我们熟悉了<code>WebRTC</code>的核心对象<code>PeerConnection</code>，也了解了核心对象的一些基本方法；后面我们也分析了<code>WebRTC</code>的会话过程，以及中间需要调用哪些方法；最后我们通过 <code>nodejs</code>搭建了最基本的信令服务器。接下来我们就开始用代码还原前面的<code>WebRTC</code>的基本会话流程。</p><p>后续<code>pc</code>代表<code>PeerConnection</code>, <code>caller</code>为<code>A</code>，<code>callee</code>为<code>B</code>举例。</p><h2 id="p2p音视频实战" tabindex="-1">P2P音视频实战 <a class="header-anchor" href="#p2p音视频实战" aria-label="Permalink to &quot;P2P音视频实战&quot;">​</a></h2><p><strong>首先，呼叫方<code>A</code>。</strong></p><blockquote><p><code>localRtcPc</code>为本地实例化后的<code>PeerConnection</code>实例，与前面整体流程有差异的地方是，我们现在在初始化 <code>pc</code> 后，直接同步获取本地摄像头和音频输入并添加到 <code>pc</code> 中。初始获取媒体流需要一定时间响应，如果在乎创建连接时间的，这一步可异步完成。</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>async initCallerInfo(callerId,calleeId){</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><ul><li>A 呼叫 B 后双方同意建立通信，A 首先初始化 <code>pc</code>，代码中的<code>localRtcPc </code>。</li></ul><ul><li>然后 A 初始化本地<code>mediaStream</code>，并添加到 pc 对象中，同时渲染在本地预览 DOM 元素。</li></ul><ul><li>初始化回调信息，比如 <code>ontrack</code>（监听B端媒体），<code>onicecandidate</code>（双方 ICE 候选信息）事件等。</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>onPcEvent(pc,localUid,remoteUid){</span></span>
...
<span class="line"><span>          }</span></span>
<span class="line"><span>        }</span></span>
<span class="line"><span>}</span></span></code></pre></div><ul><li>创建<code>offer</code>信令设置为本地描述后发送给 B 。</li></ul><ul><li>等 B 创建应答信令之后，信令服务器会将其转发到 A 这边。</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>async onRemoteAnswer(fromUid,answer){</span></span>
<span class="line"><span>    await this.localRtcPc.setRemoteDescription(answer);</span></span>
<span class="line"><span>}</span></span></code></pre></div><ul><li>A 接受 B 的 <code>answer</code>信令后，将其设置为 <code>remoteDesc </code>。</li></ul><p><img src="`+o+`" alt="" loading="lazy"></p><blockquote><p><strong>注意看日志中的<code>candidate</code>，这个过程是贯穿整个会话的，直到<code>ice</code>候选完成。</strong></p></blockquote><p><strong>被呼叫端<code>B</code>。</strong></p><blockquote><p>被呼叫端的过程和呼叫端类似，大体代码如下：</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>async initCalleeInfo(localUid,fromUid){</span></span>
<span class="line"><span>        //初始化pc</span></span>
<span class="line"><span>        this.localRtcPc = new PeerConnection()</span></span>
...
<span class="line"><span></span></span>
<span class="line"><span>    }</span></span></code></pre></div><ul><li>B 接听后同时初始化 pc。</li></ul><ul><li>B 创建本地<code>mediaStream</code>，并添加到 pc 对象中，同时渲染在本地预览 Dom 元素。</li></ul><ul><li>同 A 初始化回调监听。</li></ul><ul><li>当然此时 A 发送的<code>offer</code>信令通过信令服务器转发到 B 这边，B 将其设置为<code>remoteDesc</code>后，同时创建<code>answer</code>信令。</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>async onRemoteOffer(fromUid,offer){</span></span>
...
<span class="line"><span>    }</span></span></code></pre></div><p>至此，所有的会话建立完成，在双方监听的 pc 核心方法<code>ontrack</code>中，就能拿到双方的音频和视频信息了，完整的代码我会放在文末，大家自取即可。不过纸上得来终觉浅，还需要你自己在实践中理解，这才是掌握最快速的。</p><p><img src="`+l+'" alt="" loading="lazy"></p><p>大家可以看下下图中具体的 <code>SDP</code>信息，实际上都是<code>WebRTC</code>封装好的，不需要我们去组装，如果你认真从前面看的，应该很容易理解，完成一个 P2P 的视频通话是不是很简单？</p><p><strong>不过我先打个预防针：在真正的复杂网络环境中</strong> <strong>，</strong> <strong>我们需要考虑的还有很多，如果之前大家了解过</strong> <strong><code>WebRTC</code></strong> <strong>相关的知识，一定对</strong> <strong>stun 和 turn</strong> <strong>这几个词不陌生，我们暂时不考虑这个，</strong> <strong>从最简单的网络环境中开始</strong> <strong>，完成我们的目标。</strong></p><p><img src="'+c+`" alt="" loading="lazy"></p><p><strong>通话过程中媒体流的变更</strong></p><p>完成以上视频通话，可能有人会问，怎么实现类似微信视频中，视频和音频之间随意切换，或摄像头前置后置切换呢？</p><p>这里我们就需要再学习一个知识点：<code>RTCRtpSender</code>对象。这个对象的接口支持变更你发送到对方的媒体，通过这个对象接口，你可以编辑更改流属性，从而达到控制远端媒体流的目的。</p><p>通过实例化后的<code>PeerConnection</code>对象调用<code>getSenders</code>方法，可获取每个媒体轨道对应<code>RTCRtpSender</code>对象。这里再解释下这个媒体轨道，我们在获取到媒体信息的时候，一般包含两部分，一部分音频信息(<code>audiotrack</code>)，一部分视频信息(<code>videotrack</code>)，因此这里的媒体轨道指的就是媒体信息。</p><ul><li>音频视频模式切换。</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>//获取发送到远端的具体媒体信息的发送方信息</span></span>
...
<span class="line"><span>send.track.enabled = !send.track.enabled //控制视频显示与否 即仅音频模式</span></span></code></pre></div><ul><li>摄像头切换。</li></ul><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>//我这里web端因此只获取屏幕分享流 APP端则获取前置后置摄像头流即可</span></span>
...
<span class="line"><span>send.replaceTrack(videoTrack) //替换视频媒体信息</span></span></code></pre></div><h2 id="类im实现" tabindex="-1"><code>类IM</code>实现 <a class="header-anchor" href="#类im实现" aria-label="Permalink to &quot;\`类IM\`实现&quot;">​</a></h2><p>前面初始化回调流程中有个监听方法 <code>onPcEvent()</code>，内部你会发现有个函数<code>createDataChannel</code>，看名字就是创建了一个通道。是的，这就是的<code>WebRTC</code>中的<code>datachannel</code>可以实现无服务端 P2P 文本等富文本信息双向传输，只要完成<code>WebRTC</code>会话，即使视频通话过程中你的云服务器宕机了也没关系，P2P 的即时通讯还是可以正常进行的。我先演示下：</p><p><img src="`+t+'" alt="" loading="lazy"></p><p><strong>官方描述</strong></p><p><code>RTCPeerConnection</code> 的 <code>createDataChannel()</code> 方法可以创建一个可以发送任意数据的数据通道， 常用于后台传输内容，例如：图像、文件传输、聊天文字等其他数据，当然除了后台，最常用的就是 P2P 中客户端的双向通信了。</p><p><strong>基础语法和使用</strong></p><p>下面的创建 datachannel 的前提是双方已经完成<code>WebRTC</code>的基础信令交换，<code>pc</code>变量为初始化后的<code>RTCPeerConnection</code>。</p><blockquote><p><code>let dataChannel = RTCPeerConnection.createDataChannel(label[, options]);</code></p></blockquote><p><img src="'+i+`" alt="" loading="lazy"></p><p><strong>创建一个<code>datachannel</code>，发送并监听消息。</strong></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>this.channel =  pc.createDataChannel(&quot;my channel&quot;, {</span></span>
...
<span class="line"><span> this.channel.send(this.rtcmessage)</span></span></code></pre></div><p>通过这种方式发送消息，你在浏览器的 NetWork 是看不到的哦，因此按照常规抓包逻辑直接抓<code>HTTP</code>或者<code>WS</code>协议包的话，也是抓不到的。之前在某些网站看到过这种方式传输数据，我相信以后会有更多的地方用到<code>WebRTC</code>的<code>datachannel</code>。如果同学们想要深入了解原理，可以去看看<code>SCTP协议</code> ，点击前往<a href="http://www.watersprings.org/pub/id/draft-jesup-rtcweb-data-protocol-04.html" target="_blank" rel="noreferrer">相关协议说明官方文档地址</a>。</p><h2 id="项目操作指南" tabindex="-1">项目操作指南 <a class="header-anchor" href="#项目操作指南" aria-label="Permalink to &quot;项目操作指南&quot;">​</a></h2><ol><li>打开项目源码，找到模块：一对一网络视频。</li></ol><ol start="2"><li>进去后请在 URL 中携带参数 userId 和 roomId 。</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>#用户1001 房间号：10012</span></span>
