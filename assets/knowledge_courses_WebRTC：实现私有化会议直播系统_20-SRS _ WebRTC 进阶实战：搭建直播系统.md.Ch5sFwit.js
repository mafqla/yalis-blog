import{_ as a,o as n,c as e,a4 as p}from"./chunks/framework.CW-6O5Zu.js";const t="/assets/1.cPWEEgIs.image",l="/assets/2.Dsv-YHlU.image",i="/assets/3.jCGir6aJ.image",c="/assets/4.BW79SDnS.image",o="/assets/5.BbQZ4HUK.image",d="/assets/6.CUuBAHhK.image",r="/assets/7.DP8ytgE8.image",h="/assets/8.DUmmUqfK.image",C=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/20-SRS + WebRTC 进阶实战：搭建直播系统.md","filePath":"knowledge/courses/WebRTC：实现私有化会议直播系统/20-SRS + WebRTC 进阶实战：搭建直播系统.md","lastUpdated":1776395729000}'),u={name:"knowledge/courses/WebRTC：实现私有化会议直播系统/20-SRS + WebRTC 进阶实战：搭建直播系统.md"};function m(g,s,v,b,k,y){return n(),e("div",null,[...s[0]||(s[0]=[p('<p>上节课让大家去看看 B 站直播用的是哪种媒体流，大家去看了没有？没有也没关系，实际上现阶段基本上都是 <code>FLV</code>或者<code>HLS</code>的，毕竟大型直播以及搭配 CDN 等都有成熟的案例，而现阶段，<code>WebRTC</code>在直播场景中还是有局限性的。虽然大型直播没有，但是小型的直播还是蛮多的，而我们这节课就利用上节 <code>WebRTC</code>推流到 SRS 流媒体服务器后，再用 <code>WebRTC</code>去拉流完成直播。</p><p>当然，上节课推流后得到的两种流地址都是可以直接作为直播源的，但是在拉流速度上和<code>WebRTC</code>还有差别，接下来我们就来看看，用 <code>WebRTC</code>和 SRS 如何提高拉流的效率，大家再和 <code>FLV</code>、<code>HLS</code>拉流对比下，看下具体的差异。</p><h2 id="webrtc-拉流" tabindex="-1">WebRTC 拉流 <a class="header-anchor" href="#webrtc-拉流" aria-label="Permalink to &quot;WebRTC 拉流&quot;">​</a></h2><ol><li>获取已知要拉取的流 ID，即推流地址中的 <code>streamId</code>。你可以把这个流 ID 当作是直播间的房间号，具有唯一性。</li></ol><p><img src="'+t+`" alt="" loading="lazy"></p><ol start="2"><li>初始化 <code>WebRTC</code>核心关联对象 <code>PeerConnection</code>实例，同时监听远程媒体流。</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>const that = this</span></span>
...
<span class="line"><span>await that.pc.setLocalDescription(offer)</span></span></code></pre></div><ol start="3"><li>通过 SRS 开放 API 交换基础信令 SDP，与本地同步。</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>//组装参数 按照API格式</span></span>
...
<span class="line"><span>})</span></span></code></pre></div><ol start="4"><li>监听到媒体流后挂载到本地 <code>DOM</code> 元素。</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>setDomVideoTrick(trick){</span></span>
...
<span class="line"><span>               this.$message.error(&quot;请先点击推流&quot;)</span></span>
<span class="line"><span>       }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h3 id="音视频切换" tabindex="-1">音视频切换 <a class="header-anchor" href="#音视频切换" aria-label="Permalink to &quot;音视频切换&quot;">​</a></h3><p>这里我们使用屏幕分享来实现这个功能。</p><ol><li>获取屏幕分享流。</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>async getShareMedia(){</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><ol start="2"><li>通过核心实例化对象切换媒体流。</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>async changeVideo(){</span></span>
<span class="line"><span>       if(!this.pc){</span></span>
<span class="line"><span>               this.$message.error(&quot;请先点击推流&quot;)</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><ol start="2"><li>主播同意。同意后直接先根据观众发的流拉流即可。</li></ol><p><img src="`+i+`" alt="" loading="lazy"></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>if(e[&#39;type&#39;] === &#39;applyMic&#39;){</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><ol start="3"><li>观众端收到同意后开始推流。这一步就是普通的 WebRTC 直接推流即可。</li><li>主播端稍等即可加载出画面，开始双向通话。</li></ol><p><img src="`+c+'" alt="" loading="lazy"></p><p>至此我们的主播连麦完成了。</p><h2 id="项目演示" tabindex="-1">项目演示 <a class="header-anchor" href="#项目演示" aria-label="Permalink to &quot;项目演示&quot;">​</a></h2><ol><li>打开项目，主播访问下面模块：</li></ol><p><img src="'+o+`" alt="" loading="lazy"></p><p>但是请注意启动后台，<code>socket-server</code>文件夹中的后台。</p><p>然后携带请求参数访问：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>//指定房间号和用户ID  如果在自己的改造项目中可以写表单然后进行下一步 这里我为了演示 直接在URL携带参数</span></span>
<span class="line"><span>http://localhost:8082/srs-rtc-push?userId=999&amp;roomId=111</span></span></code></pre></div><ol start="2"><li>点击推流，右上角则会直接用 RTC 去拉流预览，成功则自动会在直播预览那里显示画面，否则会弹出失败提示框。</li></ol><p><img src="`+d+'" alt="" loading="lazy"></p><ol start="3"><li>点击麦克风或者摄像头切换，以及屏幕分享可以查看右上角预览画面变更（注意默认右上角画面是静音的，请手动开启）。</li><li>访问直播间模块页面，携带参数为推流页面的流 ID。</li></ol><p><img src="'+r+`" alt="" loading="lazy"></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>//携带个人信息+直播间流ID+tid(主播ID)</span></span>
<span class="line"><span>http://localhost:8082/srs-live-room?liveroomid=localStream-1673368291508&amp;tid=999&amp;userId=1010&amp;roomId=111</span></span></code></pre></div><ol start="5"><li>点击页面右侧“申请连麦”，观察推流模块画面以及当前页面控制台。</li></ol><p><img src="`+h+'" alt="" loading="lazy"></p><h2 id="本节相关源码" tabindex="-1">本节相关源码 <a class="header-anchor" href="#本节相关源码" aria-label="Permalink to &quot;本节相关源码&quot;">​</a></h2><p><a href="https://github.com/wangsrGit119/suke-webrtc-course/blob/main/webrtc-link-demo/src/views/srs-rtc-push.vue" target="_blank" rel="noreferrer">相关源码地址</a></p><h2 id="课后题" tabindex="-1">课后题 <a class="header-anchor" href="#课后题" aria-label="Permalink to &quot;课后题&quot;">​</a></h2><p>在申请连麦那里我做了简化，直接自动同意连麦人员画面，在实际过程中肯定是不行的，请大家优化这个步骤，比如实现主播同意、拒绝的弹窗提醒等。</p>',57)])])}const S=a(u,[["render",m]]);export{C as __pageData,S as default};
