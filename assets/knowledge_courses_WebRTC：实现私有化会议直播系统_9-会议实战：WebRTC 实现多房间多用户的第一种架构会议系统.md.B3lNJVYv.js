...
<span class="line"><span>RtcPcMaps.set(BDKeys , new PeerConnection()) //维护B-D关系</span></span></code></pre></div><p>虽然看起来很绕，但是如果我们理清楚这个逻辑，实际实现起来和 P2P 的流程一样，很简单。先看看下图，第一个就是我们前面所说的独立关联关系，第二个是美化后以及代码层面优化后的流程（<strong>即已建立关联后不再重新建立新的关联</strong>）。</p><p><img src="`+c+'" alt="" loading="lazy"></p><p>如果能明白上面的逻辑，那么我们就理解了<code>WebRTC</code>架构中最简单的，维护最方便的 <strong><code>Mesh 架构</code></strong>。这种架构不需要经过任何流媒体服务器，端对端的就可以直接实现多人视频通话，虽然简单，但是也有它的缺点，那就是<strong>昂贵的宽带，下一节我们会深入讨论。</strong></p><h2 id="代码实战" tabindex="-1">代码实战 <a class="header-anchor" href="#代码实战" aria-label="Permalink to &quot;代码实战&quot;">​</a></h2><p>我们按照下面动图流程来：</p><p><img src="'+l+`" alt="" loading="lazy"></p><ol><li>A 进入会议室：初始化信令服务器连接，然后初始化房间，如果房间中没有人，则仅获取本地媒体流并将其展示在已初始化好的 <code>DOM 节点</code> 。</li></ol><blockquote><p>roomUserList 变量为房间用户，获取全部用户后，剔出自己，然后看有无其他用户加入房间。</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>//初始化服务器连接</span></span>
...
<span class="line"><span>    //others  为空不再进行后面的,</span></span></code></pre></div><ol start="2"><li>B 用户同上：先初始化，然后获取房间用户列表。与此同时，本地获取自己的媒体流。但是与上面 A不同的是，B 加入后，A 已经在房间中，因此接下来就是 B 和 A 创建 <code>RTC 关联</code>。创建流程我们不再阐述，和之前 <code>P2P</code> 以及直播都一样，无变化。</li></ol><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes one-dark-pro material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span>async initMeetingRoomPc(){</span></span>
<span class="line"><span>        const that = this</span></span>
<span class="line"><span>        if(!this.localStream){</span></span>
<span class="line"><span>                this.localStream = await this.getLocalUserMedia()</span></span>
<span class="line"><span>                //开始静音和关闭摄像头</span></span>
...
<span class="line"><span>        })</span></span>
<span class="line"><span>},</span></span></code></pre></div><ol start="3"><li><p>C 用户进入会议室：初始化服务连接，获取房间用户，同时本地媒体流获取和预览。但房间中有 A 和 B，因此和 B 步骤一样。</p><ol><li>C 和 A 创建 <code>RTC 关联</code>。</li><li>C 和 B 创建 <code>RTC 关联</code>。</li></ol></li></ol><p><strong>根据</strong> <strong>上面的步骤</strong> <strong>，</strong> <strong>我们就可以还原如下流程了</strong> <strong>。</strong> <strong>在整个房间人员变动中，无论如何</strong> <strong>，</strong> <strong>新进来的用户都会去和房间内的其他人进行</strong> <strong>RTC</strong> <strong>关联</strong> <strong>，</strong> <strong>并实现新用户媒体流广播，从而达到我们这节课的目的：Mesh 架构的会议系统。</strong></p><p><img src="`+i+'" alt="" loading="lazy"></p><h2 id="项目操作指南" tabindex="-1">项目操作指南 <a class="header-anchor" href="#项目操作指南" aria-label="Permalink to &quot;项目操作指南&quot;">​</a></h2><ol><li>打开项目，找到模块：多对多网络视频。</li></ol><p><img src="'+r+'" alt="" loading="lazy"></p><ol start="2"><li>点击进去后首先会显示填写基础信息，要体验会议那么要加入的用户必须在同一个房间；此外，请大家注意下面图中的<code>身份ID</code>，它默认获取的是当前浏览器的指纹，因此如果在同一个浏览器不同标签页访问页面当前页面时<code>身份ID</code>都是一样的。<strong>为了避免这个问题请大家注意我访问的时候携带的参数，这个参数优先于浏览器指纹且手动指定</strong>，这样就可以避免不同用户<code>身份ID</code>一样造成加入会议失败的情况。</li></ol><p><img src="'+d+'" alt="" loading="lazy"></p><ol start="3"><li>用不同的身份ID，但是<code>同一个房间号</code>进入会议室。</li></ol><h2 id="完整代码地址" tabindex="-1">完整代码地址 <a class="header-anchor" href="#完整代码地址" aria-label="Permalink to &quot;完整代码地址&quot;">​</a></h2><p><a href="https://github.com/wangsrGit119/suke-webrtc-course/blob/main/webrtc-link-demo/src/views/demo03-many2many.vue" target="_blank" rel="noreferrer">本节课相关代码仓库地址</a></p><h2 id="课后题" tabindex="-1">课后题 <a class="header-anchor" href="#课后题" aria-label="Permalink to &quot;课后题&quot;">​</a></h2><p>在本地启动成功并模拟会议成功后，请大家尝试部署到<code>区域网</code>的某个主机上，然后通过区域网进行多人会议看看效果如何，有问题大家留言一起讨论。</p>',46)])])}const b=n(g,[["render",h]]);export{P as __pageData,b as default};
